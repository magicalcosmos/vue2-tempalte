...
import { COVERAGES, ROLES, FILESTATUS, LOGIN, LICENSESTATUS, TESTCASEKIND, VERSIONTYPE, TESTCASEGROUPTYPE } from '@/utils/dict';
...
class Common {
      ...
      UserInformation.get((error, data) => {
        ...
        if (error) {
          ...
          this._getLicenseStatus(role)
            ...
            .catch((error: any) => {
              store.dispatch('initRoutes', { licenseStatus: LICENSESTATUS.INVALID }).then(() => {
                // 重新加载路由
                router.addRoutes(store.getters.addRouters); // 动态添加可访问路由表
                reject(error);
              });
    ...
    if (typeof target === 'object') {
      ...
      if (Array.isArray(target)) {
        ...
        for (const i in target) {
          // 递归克隆数组中的每一项
          result.push(this.deepClone2(target[i]));
        }
        ...
        result = {};
        for (const i in target) {
          result[i] = this.deepClone2(target[i]);
        }
      }
  ...
  filterKeywordWhenSearch(keyword: string) {
    return keyword.replaceAll('[', '\\[').replaceAll(']', '\\]').replaceAll('(', '\\(').replaceAll(')', '\\)').replaceAll('+', '\\+').replaceAll('*', '\\*');
  }
  /**
   * 用例显示名称
   * @param item
  ...
  formatTestcaseName(item) {
    return `${item.type === TESTCASEGROUPTYPE.GROUP ? this.i18n('testcase.group') : this.i18n('overview.case')}${item.number}`;
  }
}
