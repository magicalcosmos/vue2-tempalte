...
import vue from 'vue';
import { CODELANGUAGE, TESTCASEKIND } from '@/utils/dict';
...
class VersionConfigCommon {
  ...
  getEncodingOps() {
    return [{
      label: `UTF-8(${common.i18n('common.default')})`,
      value: 'utf-8'
    }, {
      label: `GBK(${common.i18n('common.simplified_chinese')})`,
      value: 'gbk'
    }, {
      label: `GB18030(${common.i18n('common.simplified_chinese')})`,
      value: 'gb18030'
    }];
  }
  /**
  ...
  assembleData(data, thisObject: any, callback?: Function) {
    ...
    if (data.language === CODELANGUAGE['C']) {
      vue.set(data, 'languages', {
        [CODELANGUAGE['C']]: { standard: thisObject.languagesStandardsString, suffixes: CSourceSuffixes }
      });
    } else if (data.language === CODELANGUAGE['CPP']) {
      vue.set(data, 'languages', {
        [CODELANGUAGE['C']]: { standard: thisObject.languagesStandardsString, suffixes: CSourceSuffixes },
        [CODELANGUAGE['CPP']]: {
  ...
  compatibleOriginConfig(dataKey, data, thisObject: any) {
    if (!data.hasOwnProperty('staticAnalyze')) {
      ...
      const staticAnalyze = common.deepClone(data['staticAnalyze']);
      delete staticAnalyze['analyzeTemplate']; // 绕过analyzeTemplate字段
      vue.set(thisObject[dataKey], 'staticAnalyze', Object.assign(thisObject[dataKey].staticAnalyze, staticAnalyze));
    }

    if (!data.hasOwnProperty('unitTest')) {
      thisObject[dataKey].unitTest = {};
      if (data.hasOwnProperty('strictOutputCheck')) thisObject[dataKey].unitTest.strictOutputCheck = data.strictOutputCheck;
      ...
      const unitTest = common.deepClone(data['unitTest']);
      // 以下字段略过
      delete unitTest['eventReminder'];
      delete unitTest['status'];
      thisObject.unitCoverages = common.coveragesNumberToArray(data.unitTest.coverages);
      vue.set(thisObject[dataKey], 'unitTest', Object.assign(thisObject[dataKey].unitTest, unitTest));
    }
    if (data.hasOwnProperty('integrationTest')) {
      const integrationTest = common.deepClone(data['integrationTest']);
      // 以下字段略过
      delete integrationTest['eventReminder'];
      delete integrationTest['status'];
      thisObject.integrationCoverages = common.coveragesNumberToArray(data.integrationTest.coverages);
      vue.set(thisObject[dataKey], 'integrationTest', Object.assign(thisObject[dataKey].integrationTest, integrationTest));
    }
    vue.set(thisObject[dataKey], 'arguments', data['arguments']);
    vue.set(thisObject[dataKey], 'caseSensitive', data['caseSensitive']);
    vue.set(thisObject[dataKey], 'environment', data['environment']);
    vue.set(thisObject[dataKey], 'exits', data['exits']);
    vue.set(thisObject[dataKey], 'headers', data['headers']);
    vue.set(thisObject[dataKey], 'ignoreAsm', data['ignoreAsm']);
    vue.set(thisObject[dataKey], 'includeFiles', data['includeFiles']);
    vue.set(thisObject[dataKey], 'includes', data['includes']);
    vue.set(thisObject[dataKey], 'language', data['language']);
    vue.set(thisObject[dataKey], 'languages', data['languages']);
    vue.set(thisObject[dataKey], 'macros', data['macros']);
    vue.set(thisObject[dataKey], 'system', data['system']);
    vue.set(thisObject[dataKey], 'projectType', data['projectType']);
    if (data['encoding']) {
      vue.set(thisObject[dataKey], 'encoding', data['encoding']);
    }
  }
  /**
   * 处理导入的数据
  ...
  formatCoverage(coverages) {
    ...
    // coverage = coverage | TESTCASEKIND.COVERAGE_CALC_PARAM;
    return coverage;
  }
  /**
   * 复选框checkbox
  ...
  splitConfigData(dataKey: string, tempData, thisObject: any) {
    ...
    if (tempData.language === CODELANGUAGE['C']) {
    ...
    } else if (tempData.language === CODELANGUAGE['CPP']) {
      ...
      thisObject.languagesString =
        tempData.languages[CODELANGUAGE['C']] !== undefined
          ? tempData.languages[CODELANGUAGE['C']].suffixes.join(';')
          : '';
      thisObject.languagesStandardsString =
        tempData.languages[CODELANGUAGE['C']] !== undefined ? tempData.languages[CODELANGUAGE['C']].standard : '';
    }
    ...
    if (tempData.hasOwnProperty('staticAnalyze')) {
      if (tempData.staticAnalyze.analyzeTemplate) {
        if (thisObject.staticTem && thisObject.staticTem.length) {
          const staticTemMatch = thisObject.staticTem.filter(tem => tem.value === tempData.staticAnalyze.analyzeTemplate);
          if (staticTemMatch && staticTemMatch.length) {
    ...
    if (tempData.environmentId) {
      ...
      thisObject[dataKey].environmentId = '';
    }
