<template>
  <div
    ...
    <r-input
      ...
      :title="multiple ? presentText : inputValue"
      :size="realSize"
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="r-input__icon r-icon-circle-close"
    ...
    <div v-if="multiple" class="r-cascader__tags">
      <div class='multiple-tags'>
        <span
          class='multiple-tag'
          v-for="(tag, index) in presentTags"
          :key="tag.key">{{ tag.text }}{{index === presentTags.length - 1 ? '' : '; '}}</span>
      </div>
<!--      <r-tag-->
<!--        v-for="(tag, index) in presentTags"-->
<!--        :key="tag.key"-->
...
<!--      </r-tag>-->
      <input
        ...
        type="text"
        class="r-cascader__search-input"
    ...
    <transition name="r-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['r-popper', 'r-cascader__dropdown', 'max-height', popperClass]">
        <r-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></r-cascader-panel>
        <r-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="r-cascader__suggestion-panel"
          view-class="r-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'r-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="r-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="r-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
          </slot>
        </r-scrollbar>
      </div>
    </transition>
...
import RCascaderPanel from '@/components/RCascaderPanel';
import AriaUtils from '@/components/RSrc/utils/aria-utils';
import { t } from '@/components/RSrc/locale';
import { isEqual, isEmpty, kebabCase } from '@/components/RSrc/utils/util';
import { isUndefined, isFunction } from '@/components/RSrc/utils/types';
import { isDef } from '@/components/RSrc/utils/shared';
...
export default {
  ...
  data() {
    return {
      ...
      checkedValue: this.tearArray(this.value) || null,
      inputHover: false,
      inputValue: null,
      presentText: null,
      presentTags: [],
  ...
  computed: {
    realSize() {
      const _RFormItemSize = (this.RFormItem || {}).RFormItemSize;
      return this.size || _RFormItemSize || (this.$ELEMENT || {}).size;
    },
    tagSize() {
    ...
    isDisabled() {
      return this.disabled || (this.rForm || {}).disabled;
    },
    config() {
  ...
  watch: {
    ...
    value(val) {
      if (!isEqual(val, this.checkedValue)) {
        this.checkedValue = this.tearArray(val);
        this.computePresentContent();
      }
    },
    checkedValue(val) {
  ...
  methods: {
    handleSelect() {
      if (!this.inputable) {
        return this.toggleDropDownVisible(this.readonly ? undefined : true);
      }
    },
    /**
     * 拆数组
    ...
    tearArray(str) {
      ...
      const data = [str.slice(1, str.indexOf('[')), parseInt(str.slice(str.indexOf('[') + 1, str.indexOf(']')))];
      return data;
    },
    getMigratingConfig() {
      return {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.r-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.r-cascader-menu');
          firstNode = firstMenu.querySelector('.r-cascader-node[tabindex="-1"]');
        }
    ...
    computePresentText() {
      ...
      this.presentText = this.inputable ? this.value : null;
    },
    computePresentTags() {
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.r-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.r-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.r-cascader__suggestion-list');
      ...
      if (tags) {
        ...
        const height = (offsetHeight + 6) + 'px';
