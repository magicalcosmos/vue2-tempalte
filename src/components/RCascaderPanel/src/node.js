import { isEqual, capitalize } from '@/components/RSrc/utils/util';
...
export default class Node {
  ...
  getText(allLevels, separator) {
    if (separator === '[]') {
      if (this.pathLabels[0] === common.i18n('testcase.create_malloc')) {
        return '';
      }
      if (this.pathLabels[0] === 'NULL') {
        return this.pathLabels[0];
      }
      return `&${this.pathLabels[0]}` + (this.pathLabels.length === 2 ? `[${this.pathLabels[1]}]` : '');
    }
