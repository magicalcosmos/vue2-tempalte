...
import { isEqual } from '@/components/RSrc/utils/util';

const stopPropagation = e => e.stopPropagation();

export default {
  name: 'RCascaderNode',
  componentName: 'RCascaderNode',
  inject: ['panel'],
  ...
  methods: {
    ...
    renderPrefix(h) {
      ...
      } else if ((isLeaf || multiple) && isChecked) {
    ...
    renderCheckIcon(h) {
      return (
        <i class="r-icon-check r-cascader-node__postfix"></i>
      );
    },

    renderLoadingIcon(h) {
      return (
        <i class="r-icon-loading r-cascader-node__postfix"></i>
      );
    },

    renderExpandIcon(h) {
      return (
        <i class="r-icon-arrow-right r-cascader-node__postfix"></i>
      );
    },
    ...
    renderContent(h) {
      ...
      return (
        <span class="r-cascader-node__label">{ vnode || node.label }</span>
      );
    }
  },
  ...
  render(h) {
    ...
    if (expandTrigger === 'click') {
      events.on.click = e => {
        this.handleExpand();
        this.$emit('expand', e);
        if(isLeaf && multiple && !isDisabled) { // 若为当前最后一级，则执行多选选择事件
          this.handleMultiCheckChange(!isChecked);
        }
      };
    } else {
