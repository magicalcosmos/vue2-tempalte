...
import merge from '@/components/RSrc/utils/merge.js';
...
import {
  ...
  isEmpty,
  valueEquals
} from '@/components/RSrc/utils/util';
...
const getSibling = (el, distance) => {
  const { parentNode } = el;
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.r-cascader-node[tabindex="-1"]');
    ...
    handleKeyDown(e) {
      ...
      switch (keyCode) {
        ...
        case KeyCode.left:
          ...
          if (preMenu) {
            const expandedNode = preMenu.$el.querySelector('.r-cascader-node[aria-expanded="true"]');
        ...
        case KeyCode.right:
          ...
          if (nextMenu) {
            const firstNode = nextMenu.$el.querySelector('.r-cascader-node[tabindex="-1"]');
            focusNode(firstNode);
          }
          break;
        case KeyCode.enter:
    ...
    */
    ...
    scrollIntoView() {
      ...
      menus.forEach(menu => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.r-scrollbar__wrap');
          const activeNode = menuElement.querySelector('.r-cascader-node.is-active') ||
            menuElement.querySelector('.r-cascader-node.in-active-path');
