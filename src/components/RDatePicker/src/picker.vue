<template>
  ...
  <div
    class="r-date-editor r-range-editor r-input__inner"
    :class="[
      'r-date-editor--' + type,
      pickerSize ? `r-range-editor--${ pickerSize }` : '',
    ...
    v-clickoutside="handleClose"
    v-else>
    <i :class="['r-input__icon', 'r-range__icon', triggerClass]"></i>
    <input
      autocomplete="off"
    ...
    <slot name="range-separator">
      <span class="r-range-separator">{{ rangeSeparator }}</span>
    </slot>
    <input
      autocomplete="off"
    ...
    <i
      ...
      v-if="haveTrigger"
      :class="[showClose ? '' + clearIcon : '']"
      class="r-input__icon r-range__close-icon">
    </i>
  </div>
</template>
...
import { formatDate, parseDate, isDateObject, getWeekNumber } from '@/components/RSrc/utils/date-util';
...
export default {
    ...
    pickerVisible(val) {
      ...
      if (val) {
        ...
        if (this.validateEvent) {
          this.dispatch('RFormItem', 'el.form.blur');
        }
        this.$emit('blur', this);
        this.blur();
      }
  ...
  computed: {
    ...
    triggerClass() {
      return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'r-icon-time' : 'r-icon-date');
    },
