<template>
  <transition name="r-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
    <div
      ref="popper"
      v-show="visible"
      :style="{ width: width + 'px' }"
      :class="popperClass"
      class="r-picker-panel time-select r-popper">
      <r-scrollbar noresize wrap-class="r-picker-panel__content">
        <div 
...
export default {
  ...
  methods: {
    ...
    scrollToOption(selector = '.selected') {
      const menu = this.$refs.popper.querySelector('.r-picker-panel__content');
