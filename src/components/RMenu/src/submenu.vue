...
export default {
  ...
  computed: {
    ...
    menuTransitionName() {
      return this.rootMenu.collapse ? 'r-zoom-in-left' : 'r-zoom-in-top';
    },
    opened() {
    ...
    isFirstLevel() {
      ...
      while (parent && parent !== this.rootMenu) {
        if (['RSubmenu', 'ElMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
          isFirstLevel = false;
          break;
        } else {
  ...
  methods: {
    ...
    handleClick() {
      ...
      this.dispatch('RMenu', 'submenu-click', this);
    },
    handleMouseenter(event, showTimeout = this.showTimeout) {
      ...
      this.dispatch('RSubmenu', 'mouse-enter-child');
    ...
    handleMouseleave(deepDispatch = false) {
      ...
      this.dispatch('RSubmenu', 'mouse-leave-child');
      clearTimeout(this.timeout);
      this.timeout = setTimeout(() => {
      ...
      if (this.appendToBody && deepDispatch) {
        if (this.$parent.$options.name === 'RSubmenu') {
  ...
  render(h) {
    ...
    const popupMenu = (
      <transition name={menuTransitionName}>
        <div
          ref="menu"
          v-show={opened}
          class={[`r-menu--${mode}`,`r-menu-list`, popperClass]}
          ...
          <ul
            role="menu"
            class={['r-menu r-menu--popup', `r-menu--popup-${currentPlacement}`]}
    ...
    ) ? 'r-icon-arrow-down' : 'r-icon-arrow-right';
    ...
    return (
      ...
      >
        ...
        >
          {$slots.title}
          <i v-show={this.showIconArrow} class={[ 'r-submenu__icon-arrow', submenuTitleIcon ]}></i>
        </div>
