...
import RButtonGroup from '@/components/RButtonGroup';
import { generateId } from '@/components/RSrc/utils/util';

export default {
  name: 'RDropdown',

  componentName: 'RDropdown',
  ...
  watch: {
    visible(val) {
      this.broadcast('RDropdownMenu', 'visible', val);
      this.$emit('visible-change', val);
    },
    focusing(val) {
      const selfDefine = this.$el.querySelector('.r-dropdown-selfdefine');
      if (selfDefine) {
        // 自定义
        if (val) {
  ...
  methods: {
    ...
    initAria() {
      ...
      if (!this.splitButton) {
        ...
        this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' r-dropdown-selfdefine'); // 控制
      }
    },
    initEvent() {
  ...
  render(h) {
    ...
    const triggerElm = !splitButton ? (
      ...
      <r-button-group>
        <r-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick}>
          {this.$slots.default}
        </r-button>
        <r-button ref='trigger' type={type} size={dropdownSize} class='r-dropdown__caret-button'>
          <i class='r-dropdown__icon r-icon-arrow-down'></i>
        </r-button>
      </r-button-group>
    );

    return (
      <div class='r-dropdown' v-clickoutside={hide}>
        {triggerElm}
        {this.$slots.dropdown}
      </div>
    );
