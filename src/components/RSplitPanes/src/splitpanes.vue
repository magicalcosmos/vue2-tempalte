  ...
  methods: {
    ...
    checkSplitpanesNodes() {
      ...
      for (let i = 0; children && i < children.length; ++i) {
        const isPane = children[i].classList.contains('splitpanes__pane');
        const isSplitter = children[i].classList.contains('splitpanes__splitter');
      ...
      // this.container.children && this.container.children.forEach(child => {
    ...
    addSplitter(paneIndex, nextPaneNode, isVeryFirst = false) {
      ...
      // new logic for start
      if (this.panes[paneIndex] && this.panes[paneIndex].html) {
        elm.innerHTML = this.panes[paneIndex].html;
      }
      if (this.panes[paneIndex] && this.panes[paneIndex].icon) {
        elm.classList.add('new__splitter');
        const icon = document.createElement('i');

        if (this.panes[paneIndex].callback) {
          icon.onclick = this.panes[paneIndex].callback.bind(this);
        }
        icon.className = this.panes[paneIndex].icon;
        elm.append(icon);

        const newElm = document.createElement('span');
        newElm.classList.add('splitpanes__splitter__title');
        newElm.innerHTML = this.panes[paneIndex].title;
        elm.append(newElm);
      }
      // new logic for end
    ...
    requestUpdate({ target, ...args }) {
      ...
      Object.entries(args).forEach(([key, value]) => pane[key] = value); // eslint-disable-line
    },
    ...
    onPaneAdd(pane) {
      ...
      this.panes.forEach((p, i) => p.index = i); // eslint-disable-line
...
.default-theme {
  ...
  .splitpanes--vertical > .splitpanes__splitter {
    width: 7px;
    border-left: 1px solid #f2f2f280;
    margin-left: -1px;
    &:before, &:after {
