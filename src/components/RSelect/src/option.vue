...
export default {
  ...
  watch: {
    currentLabel() {
      if (!this.created && !this.select.remote) this.dispatch('RSelect', 'setSelected');
    },
    value(val, oldVal) {
      ...
      if (!this.created && !remote) {
        ...
        this.dispatch('RSelect', 'setSelected');
      }
    }
  },
  ...
  methods: {
    ...
    selectOptionClick() {
      if (this.disabled !== true && this.groupDisabled !== true) {
        this.dispatch('RSelect', 'handleOptionClick', [this, true]);
      }
    },
